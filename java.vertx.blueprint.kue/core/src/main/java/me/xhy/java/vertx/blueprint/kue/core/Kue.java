...
import me.xhy.java.vertx.blueprint.kue.core.queue.KueWorker;
...
import static me.xhy.java.vertx.blueprint.kue.core.queue.KueVerticle.EB_JOB_SERVICE_ADDRESS;
...
public class Kue {
    ...
    private void processInternal(String type, Handler<Job> handler, boolean isWorker) {
        ...
        vertx.deployVerticle(worker, new DeploymentOptions().setWorker(isWorker), r0 -> {
            if (r0.succeeded()) {
                System.out.println("deployment worker verticle success");
    ...
    public Future<Optional<Job>> getJob(long id) {
        ...
        jobService.getJob(id, r -> { // JobService.createProxy(vertx, EB_JOB_SERVICE_ADDRESS);
            System.out.println("getJob=" + id + r.succeeded() + r.cause()); // jobService.getJob 咋不好使？
            if (r.succeeded()) {
