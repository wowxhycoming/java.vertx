package me.xhy.java.vertx.microservice.portfolio.service.impl;
...
//        HttpEndpoint.getClient(discovery, new JsonObject().put("name", "CONSOLIDATION"), event -> {
    HttpEndpoint.getClient(discovery, new JsonObject().put("name", "quote"), event -> {
      if (event.failed()) {
    ...
    client.get("/?name=" + encode(company), response -> {
      ...
      if (response.statusCode() == 200) {
        response.bodyHandler(buffer -> {
          System.out.println("portfolio http get" + buffer.toJsonObject());
